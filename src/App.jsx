...
function App() {
  ...
  const fetchItems = async () => {
    try {
      ...
      // followerTrends를 배열로 변환
      const streamerListWithArray = data.map((streamer) => ({
        ...streamer,
        followerTrends: streamer.followerTrends.split(",").map(Number),
      }));

      setStreamerList(streamerListWithArray);
